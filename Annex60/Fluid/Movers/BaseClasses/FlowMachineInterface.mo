...
protected
  ...
  final parameter
    Annex60.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pCur1(
    final n = nOri,
  ...
  parameter Real powDer[size(per.power.V_flow,1)]=
   if per.use_powerCharacteristic then
     Annex60.Utilities.Math.Functions.splineDerivatives(
                   x=per.power.V_flow,
                   y=per.power.P,
                   ensureMonotonicity=Annex60.Utilities.Math.Functions.isMonotonic(x=per.power.P,
                                                                                     strict=false))
   else
...
initial algorithm
  ...
  assert(Annex60.Utilities.Math.Functions.isMonotonic(x=per.pressure.V_flow, strict=true) and
...
the simulation stops.");
  ...
  if (haveVMax and haveDPMax) or (nOri == 2) then  // ----- Curve 1
    ...
    preDer1:=Annex60.Utilities.Math.Functions.splineDerivatives(x=pCur1.V_flow,
      y=pCur1.dp);
    ...
    // used in equation 20 in Annex60/Resources/Images/Fluid/Movers/UsersGuide/2013-IBPSA-Wetter.pdf
  ...
  elseif haveVMax or haveDPMax then  // ----- Curve 2
    ...
    preDer1:=zeros(nOri);
    preDer2:=Annex60.Utilities.Math.Functions.splineDerivatives(x=pCur2.V_flow,
      y=pCur2.dp);
    preDer3:=zeros(nOri + 2);
    ...
    preDer1:=zeros(nOri);
    preDer2:=zeros(nOri + 1);
    preDer3:=Annex60.Utilities.Math.Functions.splineDerivatives(x=pCur3.V_flow,
      y=pCur3.dp);
 ...
 motDer :=if per.use_powerCharacteristic then zeros(size(per.motorEfficiency.V_flow,
    ...
    ensureMonotonicity=Annex60.Utilities.Math.Functions.isMonotonic(x=per.motorEfficiency.eta,
      strict=false));
