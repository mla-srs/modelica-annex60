...
model TestCase8 "VDI 6007 Test Case 8 model"
  ...
  Modelica.Blocks.Logical.GreaterThreshold greaterThreshold1(
    threshold=100) "Threshold for sunblind for one direction"
    annotation (Placement(transformation(
    extent={{-5,-5},{5,5}},
    rotation=-90,
    origin={-61,59})));
  Modelica.Blocks.Math.Product product1 "Solar radiation times g value for sunblind (open or closed) for
    one direction"
  ...
  Modelica.Blocks.Sources.Constant sunblind_open1(k=1)
    ...
    rotation=-90,
    origin={-27,45})));
  Modelica.Blocks.Logical.Switch switch2 "Determines g value for sunblind (open or closed) for one
    direction"
    annotation (Placement(transformation(
    extent={{-4,-4},{4,4}},
    rotation=-90,
...
equation
  ...
  "modelica://Annex60/Resources/Scripts/Dymola/ThermalZones/ReducedOrder/Validation/VDI6007/TestCase8.mos"
        "Simulate and plot"));
end TestCase8;
